...
async def answer_animation(message: Message, animation_path: str):
    ...
    if animation != None:
        await message.answer_animation(
            animation=animation["file_id"], duration=15, height=200, width=200
        )
    else:
        animation = FSInputFile(animation_path)
        msg = await message.answer_animation(
            animation=FSInputFile(animation_path), duration=15, height=200, width=200
        )
        mongodb.assets.insert_one(
            {"animation_path": animation_path, "file_id": msg.document.file_id}
        )
...
async def spin(message: Message, bot: Bot, state: FSMContext):
    ...
    if not lottery_state:
        await message.answer(
            "Столько дел навалилось на Панду Бо🐼\n\n"
    ...
    if (
        "spin_history" in lottery
        and user_id in lottery["spin_history"]
        and datetime.datetime.now().date().isoformat()
        in lottery["spin_history"][user_id]
    ):

        await message.answer(
            text=(
                "Кажется, сегодня ты уже играл🐼\n\n"
    ...
    if lottery_state["gifts"] and random.random() * 100 < lottery["gift_percent"]:
        ...
        await answer_animation(
            message=message, animation_path=f"assets/lottery/{gift}.gif"
        )
        result = gift
        if gift == "pizza":
            await message.answer(
        ...
        random.random() * 100 < lottery["bonus_point_percent"]
        and int(option) <= lottery_state["bonus_points"]["quantity"]
    ):
        lottery_state["bonus_points"]["quantity"] -= option
        lottery["bonus_points"]["quantity"] -= option
        await answer_animation(
            message=message, animation_path="assets/lottery/bonus_points_1.gif"
        )
        ...
        await message.answer(Lexicon.User.quotes[random.randint(0, 49)])
        await message.answer(
            f"Поздравляю🐼✨\n\n"
        ...
        await answer_animation(
            message=message, animation_path=f"assets/lottery/nothing_{x}.gif"
        )
        # await message.answer(
        #    "Сегодня удача прошла мимо тебя, но расстраивайся 🐼☝\n\n"
    ...
    mongodb.lottery_state.find_one_and_replace(
        filter={"label": lottery_state["label"]}, replacement=lottery_state
    )
    mongodb.lotteries.find_one_and_replace(
        filter={"label": lottery_state["label"]}, replacement=lottery
    )
