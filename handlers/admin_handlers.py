...
@router.message(
    CommandStart(), lambda message: message.from_user.id in config.tg_bot.admin_ids
)
...
async def process_load_json_game_command_load_pictures(
...
async def process_load_json_game_command_save(
...
async def process_schedule_game(message: Message, state: FSMContext):
    await message.answer(
        "Для запуска введите уникальный идентификатор игровой последовательности"
    )
...
async def process_get_label(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=create_inline_kb(
            2, {"Запуск": "run", "Тест": "test", "Отмена": "cancel"}
        ),
    )
...
async def process_select_option(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if callback.data in ["run", "test"]:
        ...
        game_callback = GameCallback(
            sequence_label=game.sequence_label, type=game.type
        ).pack()
        ...
        for user in users:
            try:
                ...
                update_user(
                    user.user_id,
                    {"last_call": f"{msg_id}|{msg_date}|{msg_chat_id}|{msg_chat_type}"},
                )
            except:
                print("МЫ В ДЕРЬМЕ!!!!!!!!!!!!!!!!!!!!")
        await state.clear()
...
async def process_stop_game_get_label(message: Message, bot: Bot, state: FSMContext):
    ...
    users = get_users()
    participants = set(
        [x.username for x in get_game_results(sequence_label=sequence_label)]
    )
    for user in users:
...
async def process_get_results_command(message: Message, state: FSMContext):
    await message.answer(
        "Для получения результатов отправьте идентификатор игровой последовательности"
    )
...
async def get_game_results_get_label(message: Message, state: FSMContext):
    ...
    if len(game_results) == 0:
        ...
        for game_result in game_results:
            answer.append(
                "@" + game_result.username + " ✅" if game_result.is_correct else " ❌"
            )
        await message.answer("\n".join(answer))
    await state.clear()
    print(await state.get_state())
...
async def launch_long_game__get_message(message: Message, state: FSMContext, bot=Bot):
    ...
    if len(months_appeared) != 1 or s.count(months_appeared[0]) != 1:
        await message.answer(
            'Извини, не могу понять, когда заканчивать игру. В сообщении не обязательно должна быть дата в формате "число месяц_в_родительном_падеже", притом только одна.'
        )
        await state.clear()
        return
        ...
        if user.last_call_giveaway == None or user.last_call_giveaway == "":
            ...
            try:
                ...
                update_user(
                    user.user_id,
                    {
                        "last_call_giveaway": f"{msg_id}|{msg_date}|{msg_chat_id}|{msg_chat_type}"
                    },
                )
        ...
        if user.last_call_giveaway != None and user.last_call_giveaway != "":
            try:
                ...
                try:
                    ...
                    print(
                        f"Невозможно удалить сообщение для пользователя {user.user_id} ({user.username})"
                    )
    ...
    for admin_id in config.tg_bot.admin_ids:
        if len(final_message) > 4096:
            for x in range(0, len(final_message), 4096):
                await bot.send_message(
                    chat_id=admin_id, text=final_message[x : x + 4096]
                )
        else:
            await bot.send_message(chat_id=admin_id, text=final_message)
...
async def process_message_users_command(message: Message, state: FSMContext):
    await message.answer(
        "Я перешлю всем пользователям следующее сообщение, которое вы мне пришлёте. Для отмены используйте команду /cancel"
    )
...
async def process_message_user_command(message: Message, state: FSMContext):
    await message.answer(
        "Для отправки сообщения пользователю мне нужен его username, например, @maks9804.\nДля отмены используйте команду /cancel"
    )
...
async def get_message_to_user_user(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Я перешлю пользователю следующее сообщение, которое вы мне пришлёте.\nДля отмены используйте команду /cancel"
    )
...
async def process_echo_command(message: Message, state: FSMContext):
    ...
    await message.answer(
        'Отправьте кнопку в формате ТЕКСТ | URL, если она нужна, и "-" в противном случае'
    )
...
async def process_edit_promos(message: Message, state: FSMContext):
    ...
    msg: Message = await message.answer(
        LEXICON["admin_edit_promos"], reply_markup=markup
    )
...
async def edit_promos_select(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if callback.data == "exit":
        ...
        print(callback.data)
        promo_status = (
            "активен" if get_promo(callback.data).label == "active" else "неактивен"
        )
        ...
        toggle_btn = (
            "Деактивировать"
            if get_promo(callback.data).status == "active"
            else "Активировать"
        )
...
async def edit_promos_choose_action(
    ...
    if callback.data == "exit":
        ...
        toggle_btn = (
            "Деактивировать"
            if get_promo(promo_label).status == "active"
            else "Активировать"
        )
        markup = create_inline_kb(
            1,
        ...
        await callback.answer(
            text="Промо-код удалён, возврат к списку кодов...", show_alert=True
        )
        promos = get_promos()
        buttons = {promo.label: promo.label for promo in promos}
        buttons.update({"Выход": "exit"})
